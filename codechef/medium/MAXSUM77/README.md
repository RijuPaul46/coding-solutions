...
**Runtime:** N/A  
**Memory:** N/A  
**Submitted:** 2026-09-02T14:43:35.961Z  

```c_cpp
#include <bits/stdc++.h>
using namespace std;
int find(vector<int>&arr,int i,int j,int k,vector<vector<int>> &dp){
    int n=arr.size();
    if(i+(n-1-j)==k)return 0;
    auto &ref=dp[i][j];
    if(ref!=-1)return ref;
    int left=arr[i]+find(arr,i+1,j,k,dp);
    int right=arr[j]+find(arr,i,j-1,k,dp);
